...
def fallo_seccion(nombre: str, err: Exception):
    ...
    st.error(f"La sección **{nombre}** no pudo mostrarse completa con los datos "
             "actuales. Las demás pestañas siguen funcionando.")
    st.caption("Sugerencias: aplica la limpieza automática en ETL, revisa que la "
               "variable objetivo tenga al menos dos clases con varios registros "
               "cada una, o prueba con otras variables.")
...
with tabs[5]:
    try:
        ...
        fallo_seccion("Variables clave", _err)
    ...
    try:
        ...
        with st.expander("⚙️ Preprocesamiento"):
            ...
            cl_enc = p2.selectbox("Codificación de categóricas", ["onehot", "ordinal"],
                                  key=wkey("cl_enc"))
            # Los límites se adaptan al tamaño real del archivo: con menos de 200
            # filas un mínimo fijo de 200 hacía fallar la pestaña.
            n_df = max(len(df), 1)
            max_rows = p3.number_input(
                "Máximo de registros", min_value=min(50, n_df),
                max_value=max(100_000, n_df), value=min(n_df, 20_000),
                step=100 if n_df < 5_000 else 500, key=wkey("cl_rows"))
        ...
        if clx is not None and vigente:
            ...
            with sub[2]:
                ...
                k_tope = int(max(2, min(30, len(M) - 1)))
                SS[wkey("km_k")] = int(min(max(int(SS.get(wkey("km_k"), 2)), 2), k_tope))
                st.number_input("Número de grupos (k)", 2, k_tope, key=wkey("km_k"),
            ...
            with sub[4]:
                ...
                k_tope_h = int(max(2, min(30, len(M) - 1)))
                SS[wkey("hc_k")] = int(min(max(int(SS.get(wkey("hc_k"), 2)), 2), k_tope_h))
                h2.number_input("Número de grupos (corte)", 2, k_tope_h, key=wkey("hc_k"))
